...
import { leaveTileNW } from "../tiles/nw/nw-move.js";
...
function nearTestNW(the_objects, ndx_x, ndx_y100, ndx_z, prev_hex, is_a_trampoline) {
    ...
    if (isVerticallyNear2(the_objects, this_hex_plus_100)) {
        move_result = leaveTileNW(the_objects, this_hex_plus_100, prev_hex, is_a_trampoline);
    } else if (isVerticallyNear2(the_objects, this_hex_plus_50)) {
        move_result = leaveTileNW(the_objects, this_hex_plus_50, prev_hex, is_a_trampoline);
    } else if (isVerticallyNear2(the_objects, this_hex_minus_50)) {
        move_result = leaveTileNW(the_objects, this_hex_minus_50, prev_hex, is_a_trampoline);
    } else if (isVerticallyNear2(the_objects, this_hex_minus_100)) {
        move_result = leaveTileNW(the_objects, this_hex_minus_100, prev_hex, is_a_trampoline);
    } else {
        move_result = leaveTileNW(the_objects, this_hex_same, prev_hex, is_a_trampoline);
    }
    return move_result;
}
