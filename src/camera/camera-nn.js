...
function nearTestNN(the_objects, ndx_x, ndx_y100, ndx_z, prev_x_ind, prev_y_ind, prev_z_ind) {
    ...
    if (prev_x_ind == ndx_x && prev_z_ind == ndx_z) {
        move_result = leaveTileNN(the_objects, this_hex_same, prev_hex);
    } else if (isVerticallyNear2(the_objects, this_hex_plus_100)) {
        move_result = leaveTileNN(the_objects, this_hex_plus_100, prev_hex);
    } else if (isVerticallyNear2(the_objects, this_hex_minus_100)) {
        move_result = leaveTileNN(the_objects, this_hex_minus_100, prev_hex);
    } else {
        move_result = leaveTileNN(the_objects, this_hex_same, prev_hex);
    }
    return move_result;
